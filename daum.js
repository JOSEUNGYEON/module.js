 ...
 function closeDaumPostcode(element_layer) {
	    ...
	    function execDaumPostcode(element_layer,i) {
    ...
    var width = 500; 우편번호서비스가 들어갈 element의 width
        var height = 500; 우편번호서비스가 들어갈 element의 height
                ...
                document.getElementById('zip_'+key).value = data.zonecode; 5자리 새우편번호 사용
				$('.zip').eq(i).val(data.zonecode);
				$('.addr1').eq(i).val(fullAddr);

                 iframe을 넣은 element를 안보이게 한다.
                 (autoClosefalse 기능을 이용한다면, 아래 코드를 제거해야 화면에서 사라지지 않는다.)
                element_layer.style.display = 'none';
            },
            width  '100%',
            height  '100%',
